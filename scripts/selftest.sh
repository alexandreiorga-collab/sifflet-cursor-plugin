...
head_ "Mutating REST API calls must ASK (they bypass every CLI gate)"
API="https://acme.siffletdata.com/api"
expect_ask "workspace apply over API"       "$(bash_ev "curl -X POST $API/v1/workspaces/abc -d @ws.json")"
expect_ask "workspace delete over API"      "$(bash_ev "curl -X DELETE $API/v1/workspaces/abc")"
expect_ask "monitor delete over API"        "$(bash_ev "curl -X DELETE $API/ui/v1/rules/abc")"
expect_ask "helper POST to workspaces"      "$(bash_ev 'scripts/sifflet-api.sh POST /v1/workspaces/abc --data @ws.json')"

head_ "Read-only API calls must NOT be blocked"
expect_silent "lineage GET"                 "$(bash_ev 'scripts/sifflet-api.sh GET /ui/v1/lineages/urn:x/downstreams')"
expect_silent "asset search (read-only POST)" "$(bash_ev "curl -X POST $API/ui/v1/assets/search -d '{}'")"
expect_silent "workspace apply dryRun=true"  "$(bash_ev "curl -X POST '$API/v1/workspaces/abc?dryRun=true' -d @ws.json")"
expect_silent "non-Sifflet API call"        "$(bash_ev 'curl -X DELETE https://api.github.com/repos/x/y')"
